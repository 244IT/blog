...
2. 通过wx.chooseImage选择图片，生成一个暂时的图片路径
...
1. 使用wx.chooseMessageFile，让用户选择朋友获取文件助手，从和朋友的聊天记录或者文件助手中获取excel表格相关数据
...
wx.chooseMessageFile({
  ...
  success: (res) => {
    ...
    this._uploadExcel(path)
  },
})
```

2. 通过wx.uploadFile上传文件到服务器

```
_uploadExcel(path) {
  const { cid } = JSON.parse(wx.getStorageSync('userInfo'))
  wx.uploadFile({
    url: 'xxxxxxx',// 上传路径
    filePath: path,
    name: 'file',
...
- 不支持使用 `eval` 执行 JS 代码
- 不支持使用 `new Function` 创建函数（实际上支持）

>以上出自官方文旦，实际上new Function所不支持的情况仅存在于一些内置函数的调用：
>
>* new Date() -> getDate()
>* new RegExp() -> getRegExp()
...
|                                                      | Android |      |
| ---------------------------------------------------- | ------- | ---- |
| String.prototype.trimStart、String.prototype.trimEnd | yes     |      |
|                                                      |         |      |
|                                                      |         |      |
|                                                      |         |      |
|                                                      |         |      |
...
|                                               | Android | ios  |
| --------------------------------------------- | ------- | ---- |
| 空值合并运算符（Nullish coalescing Operator） | yes     |      |
| 可选链 （Optional chaining）                  | yes     |      |
...
以往情况我们使用||运算符取值，当第一个值为false、0、undefined、null、NaN、空串（''）时取第二个值

```
const a = false // or 0、undefined、null、NaN、空串（''）
const b = a || 123
console.log(b) // 123
```

而空值合并运算符，只有前值为undefined和null时，才会返回第二个值。
...
const c = NaN
const d = c ?? 456
console.log(b, d) // 123 NaN
```

2.可选链

以往情况我们想要取对象深层次的值时，需要一层层判断值是否存在，否则直接取值会导致程序报错
